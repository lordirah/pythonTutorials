def load_data(spark):
    ...
    regions = spark.createDataFrame(data, columns)
    ...
    data = [('AR','Argentina',2),
            ...
            ('ZM','Zambia',4),
            ('ZW','Zimbabwe',4)]
    countries = spark.createDataFrame(data, columns)

    columns = ['location_id','street_address','postal_code','city','state_province','country_id']
    data = [(1400,'2014 Jabberwocky Rd','26192','Southlake','Texas','US'),
    ...
    locations = spark.createDataFrame(data, columns)

    columns = ['job_id','job_title','min_salary','max_salary']
    data = [(1,'Public Accountant',4200.00,9000.00),
            ...
            (20,'Test',0,1000)]
    jobs = spark.createDataFrame(data, columns)

    columns = ['department_id','department_name','location_id']
    data = [(1,'Administration',1700),
            (2,'Marketing',1800),
            (3,'Purchasing',1700),
            ...
            (10,'Finance',1700),
            (11,'Accounting',1700)]
    departments = spark.createDataFrame(data, columns)

    columns = ['employee_id','first_name','last_name','email','phone_number','hire_date','job_id','salary','manager_id','department_id']
    data = [(100,'Steven','King','steven.king@sqltutorial.org','515.123.4567','1987-06-17',4,24000.00,None,9),
    ...
    data = [(1,'Penelope','Gietz','Child',206),
    ...
    dependents = spark.createDataFrame(data, columns)

    return regions, countries, locations, jobs, departments, employees, dependents
